...
public class FormationManager {
    ...
    public FormationManager(MultiGame plugin) {
        this.plugin = plugin;
    }
    ...
    public String toggle(Player leader, Integer forcedPerRow) {
        if (!frozen.isEmpty()) {
            ...
            frozen.clear();
            return "§a[MultiGame] §f" + released + " joueur(s) libéré(s) — ils peuvent à nouveau bouger.";
        }
        ...
        if (others.isEmpty()) {
            return "§e[MultiGame] §fAucun autre joueur en ligne à placer.";
        }
        ...
        return "§a[MultiGame] §f" + n + " joueur(s) placés en grille de §e" + perRow
